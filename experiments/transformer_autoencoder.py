...
def load_cats_dataset(dataset_size):
    ...
    def lambd(filename):
        ...
        return tensor.reshape(PATH_SIZE, tokenizer.path_size).astype(np.float32)
...
_, train_set = load_cats_dataset(100)

tokenizer = SvgTokenizer(PATH_SIZE, DOTS_SIZE)

print(tokenizer.path_size)
transformer_model = Transformer(d_model=tokenizer.path_size, nhead=14, num_encoder_layers=8, num_decoder_layers=8)
...
for epoch in range(num_epochs):
    for n, real_samples in enumerate(train_loader):
        ...
        samples = real_samples.to(device=device)
        optimizer.zero_grad()
        generated_samples = transformer_model(samples, samples)
        loss = loss_function(generated_samples, samples)
        loss.backward()
        optimizer.step()
        if n % (batch_size - 1) == 0:
...
for n, real_samples in enumerate(train_loader):
    samples = real_samples.to(device=device)
    optimizer.zero_grad()
    pred = transformer_model(samples, samples)
    tensor = pred.detach().numpy()
    if count >= 64:
        break
    for tensorik in tensor:
        tokenizer.saveSvg(tensorik.reshape(PATH_SIZE, tokenizer.path_size), scale=400, filename="transformer/{}.svg".format(count))
        count = count + 1
